...
# 리스트 기능
# 1. 리스트의 길이 확인해주는 기능: len() -> 내장함수. 리스트 속 개체의 수 반환
store = ['cu', 'gs', 'seven', 'ministop']
print(len(store))

# 2. 리스트에 개체 추가[맨뒤]: append(무엇을)
store.append('emart24')
print(store)

# 3. 리스트에 개체 추가[몇번째]: insert(몇번째, 무엇을)
...
# 5. 리스트 해당 아이템의 인덱스상(순서상) 위치 찾기: index(무엇을)
print(store.index('ministop'))

# 6. 리스트에 해당 아이템 몇개 있는지 세기: count(무엇을)
print(store.count('emart24'))

# 7. 리스트를 추가: extend(리스트) +[같은역할] (뒤에 이어서 괄호안에 있는애를 추가해줌)
...
store.sort() # 알파벳 순서대로
print(store)
store.sort(reverse=True) # 리버스 하시겠습니까? 네.
print(store)
